...
Now edit your existing Compose file **directly** (you already backed it up to `docker-compose.yml.bk` in Step 1):
...
**1. Switch the application image to the Octane tag.** Find the application `image:` line and append `-octane` to the tag:

```yaml
# Before
image: "${METAFOX_APP_IMAGE:-foxsystem/metafox-fpm:8.3}"

# After
image: "${METAFOX_APP_IMAGE:-foxsystem/metafox-fpm:8.3-octane}"
```

> If your Compose file defines the image once via a shared anchor (e.g. `x-app-common: &app-common`), change it there so the `app`, `worker` and `scheduler` services all use the Octane image. If `METAFOX_APP_IMAGE` is set in your environment, make sure it points at an **`-octane`** tag.

**2. Point the Nginx service at the new Octane vhost.** In the `backend` service, change the mounted config file from the FPM vhost to the Octane vhost you created in Step 3:
...
# Before
  backend:
    ...
    volumes:
      - ./backend:/app:ro
      - ./docker/vhost-backend-nginx.conf:/etc/nginx/conf.d/default.conf:ro
    ports:
      - "8080:8080"
...
Save and exit. Leave every other service (`database`, `cache`, `memcached`, `rabbitmq`, `worker`, `scheduler`) unchanged.
...
-   **Reload after admin changes / upgrades.** Changes that affect compiled config, routes or source code (admin settings that rebuild config, app install/uninstall, or running an upgrade) require the Octane workers to reload to take effect. MetaFox handles this automatically, but if a change does not appear immediately, run:
    ```bash
    docker compose exec -u daemon app php artisan optimize
    ```
